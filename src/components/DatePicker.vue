<template>
  <div class="date__group">
    <van-field v-model="localSelectResult" is-link readonly name="picker" placeholder="选择出生日期"
      @click="showPicker = true" />
    <van-popup v-model:show="showPicker" position="bottom">
      <div class="calendar-switcher">
        <van-button :class="{ active: isGregorian }" @click="toggleCalendar('gregorian')">
          公历
        </van-button>
        <van-button :class="{ active: !isGregorian }" @click="toggleCalendar('lunar')">
          农历
        </van-button>
      </div>
      <van-picker :columns="columns" @confirm="onConfirm" @cancel="showPicker = false" @change="onChange" />
    </van-popup>
  </div>
</template>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const columns = ref([years.value, months.value, days.value, hourSegments.value]);
    ...
    const updateDaysInMonth = (year, month) => {
      ...
      columns.value = [years.value, months.value, days.value, hourSegments.value];
    };
    ...
    const updateLunarColumns = () => {
      ...
      const lunarDays = (lunarYear, lunarMonth) => {
        ...
        const daysInLunarMonth =
          (new Date(solarEnd) - new Date(solarStart)) / (1000 * 60 * 60 * 24) >= 29 ? 30 : 29;
      ...
      columns.value = [years.value, lunarMonths, days.value, hourSegments.value];
    };
    ...
    const formatLunarDate = (lunarYear, lunarMonthText, lunarDayText, hourSegment, dizhiSymbol) =>
    ...
    const onConfirm = (values) => {
      if (Array.isArray(values?.selectedValues)) {
        ...
        let formattedDate;
        if (isGregorian.value) {
          formattedDate = `${year}年${String(month).padStart(2, '0')}月${String(day).padStart(2, '0')}日 ${hourSegment}(${dizhiSymbol})`;
        } else {
          const lunarMonthText = ChineseLunar.monthName(month, 'traditional');
          const lunarDayText = ChineseLunar.dayName(day);
          formattedDate = formatLunarDate(year, lunarMonthText, lunarDayText, hourSegment, dizhiSymbol);
        }
    ...
    return {
      ...
      toggleCalendar: (type) => {
        if (type === 'gregorian') {
          ...
          updateDaysInMonth(parseInt(years.value[0].value), parseInt(months.value[0].value));
        } else if (type === 'lunar') {
